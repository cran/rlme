twonestgen = function(I,sec,mat,schdist,schpar,secdist,secpar,errdist,errpar,thetatrue){
...
#Not used in rlme
     ...
     for(i in 1:I){
      ...
      school = school[2:np1]
      class = class[2:np1]
#     xmat = rbinom(n,1,.5)	#in simulation, change it
      xmat = cbind(rbinom(n,1,.5),rnorm(n))  #trtmnt and covariate matrix
      errs = errdist(n,errpar)
      ey = xmat %*% thetatrue
      y = ey + matre[,1] + matre[,2] + errs
      list(y=y,xmat=xmat,ey=ey,matre=matre,errs=errs,school=school,section=class)
}
