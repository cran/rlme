onesampwil = function(x,test=T,alt=0,theta0=0,alpha=.05,maktable=T,plotb=T){
# This is from the ww package.
# The rlme calls this function.
...
#
    ...
    if(maktable){
        if(test){
            ...
            cat("Test-Stat. is T",ts,"Standardized (z) Test-Stat. is ",zs,"p-vlaue",pval,"\n")
            cat("\n")
        }
   ...
   list(ts=ts,zs=zs,pval=pval,est=est,lci=lci,uci=uci,tau=tau)
}
